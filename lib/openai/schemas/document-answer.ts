 ...
 * The model may only return IDs that were supplied with the evidence.
 */
...
export const documentAnswerSchema = z.object({
  answer: z
    .string()
    .describe(
      "Concise grounded answer based only on the supplied document sources. If the evidence is insufficient, say so without inventing facts.",
    ),
  supported: z
    .boolean()
    .describe(
  ...
  citations: z
    ...
    .describe(
      "Supporting source IDs when supported is true; must be empty when supported is false. Include at least one valid supplied source ID when supported is true.",
    ),
});
